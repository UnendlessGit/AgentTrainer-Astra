...
@Test func recordingPreviewUsesSealedIdentityChecksAndCausalFrameSelection() throws {
    ...
    var secondSurface = surface; secondSurface.id = "second"
    try writer.append(FrameArchive.prepare(pixels: pixels, metadata: .init(eventNanos: 140, observedNanos: 150, surface: secondSurface, byteCount: pixels.count)))
    ...
    #expect(try reader.inspect().surfaceIDs == ["fixture", "second"])
    #expect(try reader.preview(at: 175, surfaceID: "fixture")?.frame.id == frame.metadata.id)
    #expect(try reader.preview(at: 175, surfaceID: "second")?.frame.surface == secondSurface)
    #expect(try reader.preview(at: 120, surfaceID: "second") == nil)
