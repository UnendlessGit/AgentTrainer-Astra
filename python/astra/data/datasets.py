...
def build_dataset(destination: Path, *, recording_root: Path, selections: list[RecordingSelection],
    ...
    try:
        ...
        database.execute("CREATE TABLE episodes (id TEXT PRIMARY KEY, recording_id TEXT NOT NULL, split TEXT NOT NULL, steps INTEGER NOT NULL)")
        ...
        for selection, recording_id in zip(selections, ids):
            ...
            with RecordingReader(paths[recording_id]) as reader, ExitStack() as streams:
                ...
                source_hash = hashlib.sha256(_dump(manifest).encode()).hexdigest()
                ...
                sources.append({"id": recording_id, "manifestSHA256": source_hash, "split": splits[recording_id],
                                "selection": asdict(selection), "labelPartition": partition})
                ...
                def seal_episode():
                    ...
                    if episode_step:
                        ...
                        database.execute("INSERT INTO episodes VALUES(?,?,?,?)", (episode, recording_id, splits[recording_id], episode_step))
        ...
        excluded = sum(sum(source["labelPartition"]["excludedPhysicalEvents"].values()) for source in sources)
        excluded_discrete = sum(sum(source["labelPartition"]["excludedDiscreteEvents"].values()) for source in sources)
        if excluded:
        ...
        manifest = {"schemaVersion": 1, "id": identifier, "model": config.to_dict(), "actions": vocabulary.to_dict(),
...
class DatasetReader:
    def __init__(self, directory: Path, *, recording_root: Path):
        ...
        try:
            ...
            for source in manifest["sources"]:
                if not isinstance(source, dict) or set(source) != {"id", "manifestSHA256", "split", "selection", "labelPartition"}:
                ...
                if (identifier != source["id"] or identifier in self._source_specs or not isinstance(selection, dict)
                    or set(selection) != {"recording_id", "start_nanos", "end_nanos", "context_ids"}
                    or not isinstance(selection["context_ids"], list)):
                    raise RecordingError("Invalid or duplicated dataset source identity")
                chosen = RecordingSelection(**{**selection, "context_ids": tuple(selection["context_ids"])})
                ...
                _validate_partition(source["labelPartition"], self.config)
                self._source_specs[identifier] = source
    ...
    def _source(self, identifier: str) -> RecordingReader:
        ...
        if reader is None:
            ...
            selection = RecordingSelection(**{**spec["selection"], "context_ids": tuple(spec["selection"]["context_ids"])})
            first, last = _interval(reader.manifest, selection)
            if (spec["labelPartition"]["sourceStartNanos"], spec["labelPartition"]["sourceEndNanos"]) != (first, last):
                reader.close()
    ...
    def _validate_index(self):
        for table, names, keys in (
            ("episodes", ["id", "recording_id", "split", "steps"], [1, 0, 0, 0]),
        ...
        total = 0
        cursor = self._database.execute("SELECT e.*, COUNT(s.step) AS actual, MIN(s.step) AS first, MAX(s.step) AS last FROM episodes e LEFT JOIN steps s ON s.episode_id=e.id GROUP BY e.id")
        try:
            for row in cursor:
    ...
    def samples(self, episode_id: str, start: int = 0, count: int | None = None) -> Iterator[LearningSample]:
        ...
        selection = RecordingSelection(**{**source["selection"], "context_ids": tuple(source["selection"]["context_ids"])})
        ...
        try:
            for row in cursor:
                ...
                first, end = _interval(reader.manifest, selection)
