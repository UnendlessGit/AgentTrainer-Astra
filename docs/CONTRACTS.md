...
Live desktop control additionally requires a control `hello` advertising `recoveryVersion: 1`. `ArmRequest.recovery` carries a version1 `ControlRecoveryDescriptor` with `runID`, `ledgerID`, absolute `path`, `device`, and `inode`. The host creates this private 4 KiB atomic ledger before requesting arm. The acknowledgement includes `recoveryLedgerID` and `guardianPID`; both must match the mapped protected state. Older helpers must be rejected before they can ignore this extension. Unarmed diagnostics/shutdown need no ledger. See [paired recovery](architecture/0005-paired-control-recovery.md).

`TimedCommand`: offsetMs nonnegative integer; operation (`keyDown`, `keyUp`, `keyRepeat`, `buttonDown`, `buttonUp`, `pointerAbsolute`, `pointerRelative`, `scroll`); relevant arguments keyCode, button, surfaceID, x,y,dx,dy. Absolute x/y are normalized within surface globalBounds and belong to `[0,1)`, matching half-open surface ownership. Coordinate1 is rejected before admission rather than clamped after sampling. Relative dx/dy are incremental knot counts; scroll dx/dy use configured point units. Irrelevant fields are omitted. Key/button commands are idempotent; repeats on unheld keys are no-ops. Explicit keyCode flags preserve modifier sides. Absolute trajectories and relative trajectories cannot be mixed inside one packet.
...
Native LibraryStore is the sole metadata writer. Source recording files are append-only until sealed, then immutable. Agents link recordings and may save independent ordered time ranges on those links. A training job snapshots that metadata into an immutable dataset revision; later edits cannot alter an existing revision or exact resume. Compute reads sealed data and writes artifacts into allocated per-job staging paths; the coordinator atomically publishes validated manifests. No learner writes the live catalog directly. See [recording selections](verification/recording-selections.md) for the catalog-2/dataset-2 schema, legacy reader compatibility, session splits, range boundaries and byte limits.
...
An observed Secure Event Input interruption or expired keyboard verification ends recording coverage at the last trusted probe sample, even if trust returns before event delivery. Existing recording/control sessions do not silently resume across that generation boundary. Cleanup may resume with new trustworthy physical-state proof while retaining ownership until then. See [Secure Input verification](verification/secure-input.md) for the bounded probe and its limits.
