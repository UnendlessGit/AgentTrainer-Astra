...
| Build | Clean native and Python build; reproducible locked dependencies | Locked source setup; isolated d9e142b checkout passed121 active Swift checks, with two separately exercised opt-in workflows. Prior Python milestone passed250 checks with warnings treated as errors. Current guardian/adapter/experiment edits need their own integrated run; final clean release pending |
...
| RL workflows | Fresh-policy and BC-initialized runs; manual and validated automatic reset; live target continuity | Native practice PPO configuration/jobs/metrics/checkpoints integrated. Native reward definitions, local OCR/image signals, immutable catalog and recorded-sequence rehearsal implemented/tested. External adapter/assembler and guardian in development; live orchestration and reset actions pending |
...
| Experiments | Three-seed detail/temporal/cadence/hybrid comparisons and evidence-based decisions | Padding/staged-backward fixes and compiled actor measured. Matched27-update recurrent-initialization comparison remained17/32 with50% paired-cue accuracy; defaults unchanged. Credit-horizon/loss diagnostic in development. Required ablations remain open |
