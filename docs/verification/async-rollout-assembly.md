...
This is a Python assembly/admission boundary, with an independently tested native actor-record producer. A [dedicated collector worker and external learner job](collector-worker.md) now preserve process ownership and publish current actor RNG progress only after a verified source boundary. Native desktop run orchestration, production contention, and reward/reset integration remain work. The synchronous [external adapter](external-environment.md) remains available for bounded protocol tests; its exact-period restriction does not apply here.
...
The audit and rollout carry `actorProgress` schema 1: `runID`, `rngStreamID`, final sampled `drawIndex`, `rngState` after that draw, and `actorResetGeneration`. This preserves sampling consumed by the excluded suffix. The external learner job publishes the latest actor progress after its explicit boundary phase; native orchestration must restore it through an explicit new physical episode. Neither assembler nor external job silently replaces physical reset counters or claims that learner checkpoint state alone resumes a live desktop world.
